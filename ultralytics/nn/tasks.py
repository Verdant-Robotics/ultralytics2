...
class PoseSegModel(PoseModel):
    ...
    def _rasterize_boxes(self, img, gt_bboxes, gt_cls, batch_idx):
        ...
        return [
            rasterize_sorted_boxes(img, gt_bboxes_xyxy, gt_cls, batch_idx, sorted_indices, stride, self.nc)
...
class BoxInstModel(PoseModel):
    """YOLO BoxInst model: box-supervised instance segmentation on a pose-detection backbone.

    All loss computation (box/keypoint plus BoxInst's mask terms) lives in PoseLossBoxInst
    (see utils/loss.py); the model only wires up its criterion, consistent with the other
    ...
    def init_criterion(self):
        """Initialize the loss criterion for the BoxInstModel."""
        return PoseLossBoxInst(self)
